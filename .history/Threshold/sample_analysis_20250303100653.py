...
class InferenceWithThreshold:
    ...
    def generate_bounding_boxes(self, patch, properties, pred_boxes, pred_scores, criteria_labels, i_offset, j_offset):
        ...
        score_treshold = 0.8 # Has a low confidence threshold eg 0.5 to avoid false negatives and allow for manual elimination
    ...
    def process_image(self, gb):
        ...
        for prop in properties:
            ...
            nuclei_intensity = np.mean(H[mi
                                         nr:maxr, minc:maxc][cell_mask])
            ...
            if inform_file.endswith('.xlsx'):
                ...
                for image_file in relevant_images:
                    ...
                    if f'{image_name}' in errors:
                        ...
                        errors_image.append(image_name)
                        continue
                ...
                optimal_threshold, inform_data = self.calculate_threshold(valid_cells, errors_image, inform_excel)
                return  optimal_threshold, inform_data


    def calculate_threshold(self, valid_cells, errors_image, inform_data):
        ...
        print(f'Check if its the right sample {inform_data["Sample Name"].iloc[0]}')
        sample_name = inform_data["Sample Name"].iloc[0]
        #porcess_folder =  os.path.join('data/processed_thresholds',sample_name)
        # list al the files in the folder
        #files = os.listdir(porcess_folder)
        #valid_names = [f.split('.pkl')[0] for f in files]
        ...
        print(f'Number of valid cells: {len(valid_cell_ids)}')

        # Clean up the error image names to remove '_error.json'
        errors_image_cleaned = [error.replace('_error.json', '') for error in errors_image]

        # Remove cells from error images
        for error_image in errors_image_cleaned:
            inform_data = inform_data[~inform_data['Sample Name'].str.contains(error_image)]
        print(f'Number of cells after removing error images: {len(inform_data)}')

        target_phenotypes = [self.target_cell1] #CD8
        inform_data = inform_data[inform_data['Phenotype'].isin(target_phenotypes)]
        print(f'Number of cells after filtering by phenotype: {inform_data.shape[0]}')
        ...
        plt.title('Vanilla ROC curve')
        plt.legend(loc='lower right')
        plt.show()

        optimal_idx = np.argmax(tpr - fpr)
        optimal_threshold = thresholds[optimal_idx]
        print(f'Vanilla Optimal threshold for Granzyme B: {optimal_threshold}')

        # Rename the feature to avoid special characters
        inform_data.rename(columns={
        ...
        # Features and labels
        X = inform_data[['GranzymeB']].fillna(0)
        y = inform_data['True Label']

        print('Starting LightGBM model...')

        #do it with cross validation
        ...
        model = lgb.LGBMClassifier(class_weight='balanced', random_state=3)
        ...
        # Calculate ROC curve and AUC
        #fpr, tpr, thresholds_roc = roc_curve(y, predicted_probabilities)
        roc_auc = auc(fpr, tpr)
        ...
        # Convert the optimal probability threshold to the corresponding "GranzymeB" value
        optimal_threshold = np.percentile(X, optimal_threshold_prob * 100)
        print(f'Optimal threshold for Granzyme B (lighGBM): {optimal_threshold}')
        ...
        print('Starting LightGBM model with cross-validation...')
        ...
        for train, test in cv.split(X, y):
            ...
            tpr_interp = interp1d(fpr, tpr, kind='linear', bounds_error=False, fill_value=0)(mean_fpr)
            tpr_interp[0] = 0.0
        ...
        plt.figure()
        plt.plot(mean_fpr, mean_tpr, color='purple', label=r'Mean ROC (AUC = %0.2f $\pm$ %0.2f)' % (mean_auc, std_auc))
        plt.fill_between(mean_fpr, np.maximum(mean_tpr - np.std(tprs, axis=0), 0), 
                        np.minimum(mean_tpr + np.std(tprs, axis=0), 1), color='grey', alpha=0.2, label=r'$\pm$ 1 std. dev.')
        plt.plot([0, 1], [0, 1], linestyle='--', color='navy')
        plt.xlim([0.0, 1.0])
        plt.ylim([0.0, 1.05])
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
        plt.title('ROC Curve with Cross-Validation (LightGBM)')
        plt.legend(loc='lower right')
        plt.show()

        #Calculate the roc with the minimum granzyme b value as threshold
        min_granzyme_b = min(granzyme_b_values)
        print(f"Minimum Granzyme B value as threshold: {min_granzyme_b}")
        predicted_labels_min = [1 if value >= min_granzyme_b else 0 for value in granzyme_b_values]
        fpr, tpr, thresholds = roc_curve(true_labels, predicted_labels_min)
        roc_auc = auc(fpr, tpr)
        ...
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
        plt.title('ROC curve with Minimum Granzyme B Threshold')
        plt.legend(loc='lower right')
        plt.show()
...
class GranzymeBAnalysis:
    ...
    def visualize_granzyme_b_distribution(self):
        ...
        plt.axvline(self.threshold, color='r', linestyle='--', label='Threshold  {0:.2f}'.format(self.threshold))
        plt.legend()
    ...
    def classify_and_visualize(self):
        ...
        for target_label in target_labels:
            ...
            GranzymeB = target_data['GranzymeB']
            Quadrant_1 = target_data[(GranzymeB >= self.threshold)].shape[0] / target_data.shape[0]
            Quadrant_4 = target_data[(GranzymeB < self.threshold)].shape[0] / target_data.shape[0]
...
processed_data_folder = 'data/refine_model_processed_thresholds'
