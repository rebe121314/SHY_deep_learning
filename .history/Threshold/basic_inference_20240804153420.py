...
# Connect to Dropbox
ACCESS_TOKEN = 'sl.B6ULxLW7XN8aRxOZq2NPqqBAYAuqBvnQsJdgEUD3HzQb3QLetY4e22sGeoATYMDzqntX99EffWED_WoAHP_thg9tkLhCJBh5lZnjNjkQa1PeeJvNnvkJ5tZDJ7rUt-l8ABtYouDyHYLa'
...
class Inference:
    ...
    def full_processing(self, image, patch_size, criteria_labels, image_name):
        ...
        if self.manual:
            updated_boxes = self.manual_elimination(new_image, all_boxes)
            num_epochs = 1
            self.reinforcement_learning(self.model, image, all_boxes, updated_boxes, self.device, num_epochs)
    ...
    def manual_elimination(self, image, boxes):
        ...
        return [boxes[i] for i in range(len(boxes)) if i not in selected_boxes]


    def reinforcement_learning(self, model, image, original_boxes, updated_boxes, device, num_epochs):
        # Create pseudo-labels from manual corrections
        pseudo_labels = []
        ...
        for epoch in range(num_epochs):
            ...
            for box, label in pseudo_labels:
                ...
                # Extract patch based on bounding box
                patch = self.get_patch_from_box(image, x_min, y_min, x_max, y_max)
                patch = patch.unsqueeze(0).to(device)

                optimizer.zero_grad()
                output = model(patch)
                loss = loss_fn(output, torch.tensor([label], dtype=torch.long).to(device))
                loss.backward()
                optimizer.step()
    ...
    def get_patch_from_box(self, image, x_min, y_min, x_max, y_max):
