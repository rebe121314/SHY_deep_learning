...
class GranzymeBAnalysis:
    ...
    def classify_and_visualize(self, model):
        ...
        for target_label in target_labels:
            ...
            plt.scatter(target_data['GranzymeB'], target_data['CD8_Nuc'], s=0.1, c=target_data['Predicted Label'], cmap='coolwarm')
            plt.xlim(0.00005, 5)
            plt.ylim(0.0000005, 5)
            plt.axhline(y=self.threshold, color='r', linestyle='dashed')
            plt.xscale("log")
            plt.yscale("log")
            plt.xlabel('Granzyme B Mean Intensity', fontsize=25)
            plt.ylabel('CD8 Mean Intensity', fontsize=25)
            plt.title(f'For {target_label} cell', fontsize=30)
            ...
            fig = plt.figure(figsize=(8, 6)) 
            plt.scatter(target_data['GranzymeB'], target_data['DAPI'], s=0.1, c=target_data['Predicted Label'], cmap='coolwarm')
            plt.xlim(0.00005, 5)
            plt.ylim(0.0000005, 5)
            plt.axhline(y=self.threshold, color='r', linestyle='dashed')
            plt.xscale("log")
            plt.yscale("log")
            plt.xlabel('Granzyme B Mean Intensity', fontsize=25)
            plt.ylabel('DAPI Mean Intensity', fontsize=25)
