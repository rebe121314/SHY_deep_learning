...
class inFormModelEval:
        ...
        for _, row in relevant_rows.iterrows():
            if row['Cell ID'] not in done:
                if row['Label'] == 'gb':
                    ...
                    print(row['Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting)'])
                    gb = int(row['Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting)'])
    ...
    def evaluate_and_visualize(self):
        ...
        sns.heatmap(cm_percentage, annot=True, fmt='.2%', cmap='coolwarm', xticklabels=['Predicted Positive', 'Predicted Negative'], yticklabels=['Actual Positive', 'Actual Negative'])
