...
class inFormModelEval:
    ...
    def generate_bounding_boxes(self, patch, criteria_labels, pred_boxes, pred_scores,i_offset, j_offset):
        ...
        for _, row in relevant_rows.iterrows():
            self.total_gb_val.append(row['Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting)'])
    ...
    def evaluate_and_visualize(self):
        ...
        if NM > 0 and TP > 0:
            ...
            for i in range(len(self.score_gb)):
                if self.score_gb[i] > 0.5:
                    gb_50.append(self.truegb_val[i])
                    score_50.append(self.score_gb[i])

            print(f"Number of True Positives with Score > 0.5: {len(gb_50)}")
        ...
        if TP > 0 and len(self.truegb_val) > 1 and len(self.score_gb) > 1:
            ...
            if len(gb_50) > 0:
                ...
                plt.ylabel('Granzyme B Values')
                plt.title('Correlation between  True Granzyme B Values and Predicted Probability (Scores > 0.5)')
                plt.legend()
                plt.show()
        ...
        return precision, recall, f1, cm, self.scores
