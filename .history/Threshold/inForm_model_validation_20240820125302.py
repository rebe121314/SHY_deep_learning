...
class inFormModelEval:
    ...
    def generate_bounding_boxes(self, patch, criteria_labels, pred_boxes, pred_scores,i_offset, j_offset):
        ...
        for pred_box, score in zip(sorted_pred_boxes, sorted_pred_scores):
            ...
            for _, row in relevant_rows.iterrows():
                ...
                gb = int(row['Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting)'])
    ...
    def evaluate_and_visualize(self):
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='rocket', xticklabels=['Predicted Positive', 'Predicted Negative'], yticklabels=['Actual Positive', 'Actual Negative'])
        ...
        sns.heatmap(cm_percentage, annot=True, fmt='.2%', cmap='rocket', xticklabels=['Predicted Positive', 'Predicted Negative'], yticklabels=['Actual Positive', 'Actual Negative'])
        ...
        for i in range(5):
            ...
            for inform_file in inform_files:
                if inform_file.endswith('.xlsx'):
                    ...
                    relevant_images = np.random.choice(relevant_images, 2)
