    ...
    def evaluate_and_visualize(self):
        ...
        if NM > 0 and TP > 0:
            ...
            for i in range(len(self.score_gb)):
                if self.score_gb[i] > 0.5 and gb_mdeian >= self.truegb_val[i]:
                    gb_50.append(self.truegb_val[i])
                    score_50.append(self.score_gb[i])

            print(f"Number of True Positives with Score > 0.5 (and larger than the lower 10% of the distribution): {len(gb_50)}")
