...
class inFormModelEval:
    ...
    def run_inference_on_patches(self, patches):
        ...
        for patch, i, j in patches:
            ...
            self.scores.append(score for score in pred_scores)
    ...
    def generate_bounding_boxes(self, patch, criteria_labels, pred_boxes, pred_scores,i_offset, j_offset):
        ...
        print('Here')
        relevant_rows['Cell X Position'] = pd.to_numeric(row['Cell X Position'])
        relevant_rows['Cell Y Position'] = pd.to_numeric(row['Cell Y Position'])
        x_max_patch = j_offset + patch.shape[1]
        y_max_patch = i_offset + patch.shape[0]
        x_min_patch = j_offset
        y_min_patch = i_offset
        relevant_rows = relevant_rows[(int(relevant_rows['Cell X Position']) >= x_min_patch) & (int(relevant_rows['Cell X Position']) <= x_max_patch)]
        relevant_rows = relevant_rows[(int(relevant_rows['Cell Y Position']) >= y_min_patch) & (int(relevant_rows['Cell Y Position']) <= y_max_patch)]
        ...
        for pred_box, score in zip(sorted_pred_boxes, sorted_pred_scores):
            ...
            for _, row in relevant_rows.iterrows():
                x_position = int(row['Cell X Position'])
                y_position = int(row['Cell Y Position'])
                ...
                if expanded_minc <= x_position <= expanded_maxc and expanded_minr <= y_position <= expanded_maxr:
                    ...
                    if row['Label'] == 'gb':
                        ...
                        self.truegb_val.append(int(row['Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting']))
                    else:
                ...
                if row['Label'] == 'gb':
                    ...
                    self.non_match_val.append(int(row['Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting']))
                else:
                    self.false_positives += 1
    ...
    def evaluate_and_visualize(self):
        ...
        TN = self.not_gb_fp + self.not_phen_fp  # Assuming these are true negatives
        ...
        if NM > 0:
            ...
            FN += len(reclassified_fn)  # Update FN for subsequent calculations
