...
class inFormModelEval:
    ...
    def evaluate_and_visualize(self):
        ...
        if TP > 0:
            ...
            corr, p_value = pearsonr(self.truegb_val, self.scores)

            # Create the scatter plot
            plt.figure(figsize=(8, 6))
            sns.scatterplot(x=self.scores, y=self.truegb_val, color='blue', alpha=0.5)

            # Fit a linear line to the scatter plot
            m, b = np.polyfit(self.scores, self.truegb_val, 1)
            plt.plot(self.scores, m * np.array(self.scores) + b, color='orange', label=f'Fit line (slope={m:.2f})')
            ...
            plt.title('Correlation between Granzyme B Values and Predicted Probability')
    ...
    def run_model_val(self):
        ...
        for i in range(3):
            inform_files = np.random.choice(inform_files_main, 5)
