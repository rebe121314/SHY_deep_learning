...
class inFormModelEval:
    ...
    def model_eval(self, criteria_labels, valid_boxes, predicted_boxes):
        ...
        for pred_box in predicted_boxes:
            x_min, y_min, x_max, y_max = pred_box['Bounding Box']
            matched = False
        ...
        # Count criteria labels that were not matched by any prediction, without assuming they are false negatives
        for criteria_df in criteria_labels:
            for _, row in criteria_df.iterrows():
                x_position = row['Cell X Position']
                y_position = row['Cell Y Position']
    ...
    def run_model_val(self):
            ...
            if inform_file.endswith('.xlsx'):
                ...
                for image_file in relevant_images:
                    ...
                    try:
                        ...
                        valid_boxes, all_boxes  = self.full_processing(gb_image, self.patch_size, criteria_labels, image_name)
                        valid_cells.extend(valid_boxes)
                        predicted_cells.extend(all_boxes)
                        excel_files.extend(criteria_labels)
        ...
        self.model_eval(self.model, excel_files, valid_cells, predicted_cells)
