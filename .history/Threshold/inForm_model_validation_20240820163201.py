...
class inFormModelEval:
    ...
    def evaluate_and_visualize(self):
        ...
        if NM > 0 and TP > 0:
            ...
            plt.axvline(mean_tp, color='green', linestyle='--', label=f'Mean TP: {mean_tp:.4f}')
            plt.axvline(mean_nm, color='orange', linestyle='--', label=f'Mean NM: {mean_nm:.4f}')
            plt.axvline(lowest_tp_value, color='red', linestyle='--', label='Lowest TP Value')
            ...
            plt.axvline(log_mean_tp, color='green', linestyle='--', label=f'Log Mean TP: {log_mean_tp:.4f}')
            plt.axvline(log_mean_nm, color='orange', linestyle='--', label=f'Log Mean NM: {log_mean_nm:.4f}')
            plt.axvline(np.log10(lowest_tp_value), color='red', linestyle='--', label='Lowest TP Value')
        ...
        sns.heatmap(cm, annot=True, fmt='.2f', cmap='coolwarm', xticklabels=['Predicted Positive', 'Predicted Negative'], yticklabels=['Actual Positive', 'Actual Negative'])
        plt.title('Confusion Matrix')
        plt.show()

        #cm = confusion_matrix(
        #    [1] * TP + [0] * (FP + TN) + [1] * FN,  # True labels (excluding NM)
        #    [1] * TP + [1] * FP + [0] * TN + [0] * FN,  # Predicted labels (excluding NM)
        #    labels=[1, 0]
        #)
        #sns.heatmap(cm, annot=True, fmt='d', cmap='plasma', xticklabels=['Predicted Positive', 'Predicted Negative'], yticklabels=['Actual Positive', 'Actual Negative'])
        #plt.title('Confusion Matrix')
        #plt.show()
        ...
        sns.heatmap(cm_percentage, annot=True, fmt='d', cmap='coolwarm', xticklabels=['Predicted Positive', 'Predicted Negative'], yticklabels=['Actual Positive', 'Actual Negative'])
        ...
        return precision, recall, f1, cm, self.scores, reclassified_fn
    ...
    def run_model_val(self):
        ...
        for i in range(1):
            inform_files = np.random.choice(inform_files_main, 1)
...
models_to_analyze = ['data/saved_models/10e_trans_with_relax.pth', 'data/saved_models/20e_trans_with_relax.pth', 'data/saved_models/copy_20e_trans_with_relax.pth', 'data/saved_models/with_trans_longish.pth']
