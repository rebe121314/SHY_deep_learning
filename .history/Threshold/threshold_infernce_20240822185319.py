...
class InferenceWithThreshold:
    ...
    def generate_bounding_boxes(self, patch, pred_boxes, pred_scores, criteria_labels, i_offset, j_offset):
        ...
        criteria_labels['Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting)'] = pd.to_numeric(criteria_labels['Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting)']).dropna()
        ...
        for pred_box, score in zip(sorted_pred_boxes, sorted_pred_scores):
            ...
            for _, row in criteria_labels.iterrows():
                row['Cell X Position'] = pd.to_numeric(row['Cell X Position'], errors='coerce')
