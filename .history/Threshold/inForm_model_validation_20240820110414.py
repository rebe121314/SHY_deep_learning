...
class inFormModelEval:
        ...
        for index, row in relevant_cells.iterrows():
            if row['Phenotype'] in pos_phen:
                if row[gb_ent] > 0 and row[gb_cyt] > 0:
                        if row[tres_cd8_ent] >= row[cd8_ent]:
    ...
    def evaluate_and_visualize(self):
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='viridis', xticklabels=['Predicted Positive', 'Predicted Negative'], yticklabels=['Actual Positive', 'Actual Negative'])
        ...
        sns.heatmap(cm_percentage, annot=True, fmt='.2%', cmap='viridis', xticklabels=['Predicted Positive', 'Predicted Negative'], yticklabels=['Actual Positive', 'Actual Negative'])
