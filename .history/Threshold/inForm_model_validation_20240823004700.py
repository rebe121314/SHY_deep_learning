...
class inFormModelEval:
    ...
    def generate_bounding_boxes(self, patch, criteria_labels, pred_boxes, pred_scores,i_offset, j_offset):
        ...
        lower_10_percentile = np.percentile(relevant_rows['Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting)'], 10)
        ...
        for pred_box, score in zip(sorted_pred_boxes, sorted_pred_scores):
            ...
            for _, row in relevant_rows.iterrows():
                ...
                if expanded_minc <= x_position <= expanded_maxc and expanded_minr <= y_position <= expanded_maxr:
                    ...
                    if (x_position, y_position) not in done_point:
                        boxes.append({
                            'Cell ID': row['Cell ID'],
    ...
    def evaluate_and_visualize(self):
        ...
        if NM > 0 and TP > 0:
            ...
            score_low_percentile = np.percentile(self.score_gb, 10)


            gb_50 = self.truegb_val
            
            score_50 = self.score_gb
            ...
            # Set a significance level for the p-value
            alpha = 0.05  # 5% significance level
            lowest_tp_value = 0

            # Identify non-matched values that should be reclassified as false negatives using Mann-Whitney U test
            for val in non_matched_values:
                # Perform the Mann-Whitney U test
                u_stat, p_value = mannwhitneyu(true_positive_values, [val], alternative='less')

                # If the p-value is below the significance level, classify as a false negative
                if p_value < alpha:
            ...
            print(f"Number of statistical false negative: {FN}")
                ...
                for image_file in relevant_images:
                    ...
                    if processed_data:
                        ...
                        try:
                            ...
                            if human_in_loop:
                                self.human_in_the_loop_update(gb_image, valid_boxes, self.patch_size)
                            self.save_processed_data(name_sample, image_name, valid_boxes)
...
inference = inFormModelEval(inform_files, granzyme_b_image_folder, processed_data_folder, model_path, patch_size)
