...
class GranzymeBDataset(Dataset):
    ...
    def _load_images_and_labels(self):
        ...
        label_train = label_files[:int(len_f*0.8)]
        label_val = label_files[int(len_f*0.2):]
        if self.val:
            # Use the first 6 samples for validation
            label_files = label_val
        else:
...
def train(model, data_loader, val_loader, optimizer, scheduler, device, num_epochs):
    ...
    plot_prec_histogram(metrics["precision"])
...
if __name__ == "__main__":
    ...
    load_path = "data/saved_models/original_more_epoch_extra_relax_basic.pth"
