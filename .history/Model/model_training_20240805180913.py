...
class GranzymeBDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        image = self.db_handler.read_image_from_dropbox(img_path)

        label_path = f"{self.labels_dir}/{label_name}"
        label_data = self.db_handler.read_json_from_dropbox(label_path)

        boxes = [box["Bounding Box"] for box in label_data]

        patches = ImageProcessor.create_patches(image, boxes, self.patch_size)
...
if __name__ == "__main__":
    ...
    labels_folder = '/Labels/manual_box_label'
    patch_size = 256
    ...
    load_path = "data/saved_models/simpie_then_manual.pth"
    ...
    save_path = "data/saved_models/simpie_then_manual.pth"
