...
def model_evaluation(model, data_loader, device):
    ...
    for image_patches, target_patches in data_loader:
        for image, target in zip(image_patches, target_patches):
            ...
            patch_metric = MeanAveragePrecision(iou_thresholds=[0.5], class_metrics=True)
...
def plot_patch_metrics(patch_metrics, overall_result):
    ...
    plt.axvline(x=overall_result['map'].cpu().item(), color='red', linestyle='--', label="Overall mAP")
    ...
    plt.subplot(1, 3, 2)
    plt.hist(map_50s, bins=10, color='orange', alpha=0.7)
    plt.axvline(x=overall_result['map_50'].cpu().item(), color='red', linestyle='--', label="Overall mAP_50")
    plt.xlabel('mAP_50 Value')
    plt.ylabel('Frequency')
    plt.title('Histogram of mAP_50 per Patch')
    plt.legend()

    plt.subplot(1, 3, 3)
    plt.hist(recalls, bins=10, color='green', alpha=0.7)
    plt.axvline(x=overall_result['mar_100_per_class'].mean().cpu().item(), color='red', linestyle='--', label="Overall Recall")
