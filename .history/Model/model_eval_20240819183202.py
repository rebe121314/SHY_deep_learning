...
def calculate_model_metrics(model, image, target, device, iou_threshold=0.5):
    ...
    # Precision-Recall
    if len(np.unique(metrics["tp"])) > 1:
        precision, recall, _ = precision_recall_curve([1]*metrics["tp"] + [0]*(metrics["fp"]+metrics["fn"]), pred_scores)
        metrics["precision"].extend(precision)
        metrics["recall"].extend(recall)
    else:
        metrics["precision"].extend([0])
...
# Update the model evaluation function to include confusion matrix and mAP
def model_evaluation(model, data_loader, device, iou_threshold=0.5):
    model.eval()
    all_true_labels = []
    all_pred_labels = []
    metrics = {"precision": [], "recall": [], "iou": [], "mAP": []}
    ...
    for image_patches, target_patches in data_loader:
        for image, target in zip(image_patches, target_patches):
            ...
            # Calculate metrics
            epoch_metrics = calculate_model_metrics(model, image, target, device, iou_threshold)
            metrics["precision"].extend(epoch_metrics["precision"])
            metrics["recall"].extend(epoch_metrics["recall"])
            metrics["iou"].extend(epoch_metrics["iou"])
            metrics["mAP"].append(epoch_metrics["mAP"])

            # Collect data for confusion matrix
            pred_labels = [1 if score > 0.5 else 0 for score in prediction["scores"].cpu().numpy()]
            true_labels = [1] * len(target["boxes"])
