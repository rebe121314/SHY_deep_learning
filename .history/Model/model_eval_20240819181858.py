...
def model_evaluation(model, data_loader, device, iou_threshold=0.5):
        ...
        for batch in data_loader:
            ...
            for image, target in zip(image_patches, target_patches):
                ...
                for _ in range(false_negatives):
                    all_true_labels.append(1)  # False negative
                    all_pred_scores.append(0)  # No corresponding prediction
    ...
    # Check if we have both positive and negative labels for meaningful ROC and PR curves
    if len(np.unique(all_true_labels)) > 1:
        ...
        print("Skipping ROC, PR curve, and confusion matrix because only one class is present.")
        print(np.unique(all_true_labels))

    return precision_mean, recall_mean, iou_mean, mAP
...
def plot_roc_curve_with_ci(fpr, tpr, auc_score, n_bootstraps=1000, alpha=0.95):
    ...
    plt.fill_between(fpr, np.interp(fpr, fpr, tpr) - (tpr - np.interp(fpr, fpr, tpr)), 
                     np.interp(fpr, fpr, tpr) + (tpr - np.interp(fpr, fpr, tpr)),
                     color='grey', alpha=0.3, label=f'95% CI: [{ci_lower:.2f} - {ci_upper:.2f}]')
