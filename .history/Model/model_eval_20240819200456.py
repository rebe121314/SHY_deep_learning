...
def model_evaluation(model, data_loader, device):
    ...
    for image_patches, target_patches in data_loader:
        for image, target in zip(image_patches, target_patches):
            ...
            preds = [{
                ...
                "labels": torch.ones(len(prediction["boxes"])).int()
            }]

            targets = [{
                "boxes": target["boxes"].cpu(),
                "labels": torch.ones(len(target["boxes"])).int()
            }]
    ...
    # Plot Precision-Recall curve
    plot_precision_recall_curve(result['map_per_class'], result['map'])

    # Plot IoU histogram
    plot_iou_histogram(result['map_iou'])
...
def plot_precision_recall_curve(precision, recall):
    plt.figure(figsize=(10, 5))
    plt.plot(recall, precision, marker='.', color='purple', label=f'mAP={precision:.2f}')
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Precision-Recall Curve')
    plt.legend()
