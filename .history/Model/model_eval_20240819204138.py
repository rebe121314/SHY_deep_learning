...
def model_evaluation(model, data_loader, device):
    ...
    # Initialize lists to collect metrics for all images
    all_precisions = []
    all_recalls = []
    all_ious = []
    all_maps = []
    ...
    for image_patches, target_patches in data_loader:
        for image, target in zip(image_patches, target_patches):
            ...
            preds = [{
                ...
                "labels": torch.ones(len(prediction["boxes"])).int()  # Assuming a single class (Granzyme B)
            }]

            targets = [{
                "boxes": target["boxes"].cpu(),
                "labels": torch.ones(len(target["boxes"])).int()  # Assuming a single class (Granzyme B)
            }]

            # Update metric for the current image/patch
            metric.update(preds, targets)

            # Compute metrics for the current image/patch
            result = metric.compute()

            # Collect metrics
            all_precisions.append(result['map_per_class'].cpu().numpy())
            all_recalls.append(result['mar_100_per_class'].cpu().numpy())
            all_ious.append(result['map_50'].cpu().numpy())
            all_maps.append(result['map'].cpu().numpy())

    # After all images/patches are processed, plot the results

    # Plot Precision-Recall curve
    plot_precision_recall_curve_new(all_precisions, all_recalls)

    # Plot IoU histogram
    plot_iou_histogram(all_ious)

    # Plot mAP histogram
    plot_map_histogram(all_maps)

    return result

def plot_precision_recall_curve_new(all_precisions, all_recalls):
    plt.figure(figsize=(10, 5))
...
def plot_iou_histogram(all_ious):
    iou_values = [iou for ious in all_ious for iou in ious]  # Flatten list of lists
    plt.figure(figsize=(10, 5))
...
def plot_map_histogram(all_maps):
    plt.figure(figsize=(10, 5))
    plt.hist(all_maps, bins=50, color='purple', alpha=0.7)
    plt.xlabel('mAP')
    plt.ylabel('Frequency')
