...
class GranzymeBDataset(Dataset):
    ...
    def _load_images_and_labels(self):
        ...
        if self.val:
            ...
            #make random
            label_files = sample(label_files, int(len_f*0.2))
...
# Function to plot the ROC curve with a confidence interval
def plot_roc_curve_with_ci(fpr, tpr, auc_score, n_bootstraps=1000, alpha=0.95):
    plt.figure(figsize=(10, 5))
    ...
    for i in range(n_bootstraps):
        # Resample with replacement from the indices
        indices = np.random.randint(0, len(fpr), len(fpr))
        if len(np.unique(indices)) < 2:
            continue  # skip this resample if there are fewer than 2 unique points
        
        # Use these indices to resample fpr and tpr
        fpr_resampled = fpr[indices]
        tpr_resampled = tpr[indices]
        
        # Sort fpr_resampled and tpr_resampled based on fpr to ensure a proper ROC curve shape
        order = np.argsort(fpr_resampled)
        fpr_resampled = fpr_resampled[order]
        tpr_resampled = tpr_resampled[order]

        # Interpolate tpr_resampled to match base_fpr
        tpr_interpolated = np.interp(base_fpr, fpr_resampled, tpr_resampled)
    ...
    ci_lower = np.percentile(tprs, (1.0 - alpha) / 2.0 * 100, axis=0)
    ci_upper = np.percentile(tprs, (1.0 + alpha) / 2.0 * 100, axis=0)
    
    # Plot the mean ROC curve
    plt.plot(base_fpr, mean_tpr, color='purple', label=f'ROC curve (area = {auc_score:.2f})')

    # Fill the confidence interval
    plt.fill_between(base_fpr, ci_lower, ci_upper, color='grey', alpha=0.3, label=f'95% CI')
...
def model_evaluation(model, data_loader, device):
    ...
    with torch.no_grad():
        for batch in data_loader:
            # Each batch contains a list of image patches and a list of corresponding target patches
            image_patches, target_patches = batch
            ...
            for image, target in zip(image_patches, target_patches):
                ...
                # Flatten true labels and predicted scores
                true_labels = target["labels"].cpu().numpy().flatten()
                pred_scores = prediction["scores"].cpu().numpy().flatten()

                if len(true_labels) < len(pred_scores):
                    # Select top-N predictions where N = number of true labels
                    indices = np.argsort(pred_scores)[-len(true_labels):]
                    pred_scores = pred_scores[indices]
                    pred_boxes = prediction["boxes"].cpu().numpy()[indices]
                elif len(true_labels) > len(pred_scores):
                    print(f"Skipping due to length mismatch: {len(true_labels)} true labels vs {len(pred_scores)} pred scores")
                    continue
    ...
    if len(np.unique(all_true_labels)) > 1:  # Ensure there are both classes present
        ...
        roc_auc = auc(fpr, tpr)
        cm = confusion_matrix(all_true_labels, (all_pred_scores > 0.5).astype(int))
