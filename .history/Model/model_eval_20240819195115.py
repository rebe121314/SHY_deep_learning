...
def model_evaluation(model, data_loader, device, iou_threshold=0.5):
    ...
    for image_patches, target_patches in data_loader:
        for image, target in zip(image_patches, target_patches):
            ...
            # Collect all true boxes and predicted boxes across the dataset
            true_boxes = target["boxes"].cpu().numpy()
            pred_boxes = prediction["boxes"].cpu().numpy()
            pred_scores = prediction["scores"].cpu().numpy()
            ...
            aggregated_metrics["iou"].extend(ious)

            # mAP Calculation (averaged over the entire dataset)
            map_metric = MeanAveragePrecision()
            map_metric.update([{
                "boxes": torch.tensor(pred_boxes),
                "scores": torch.tensor(pred_scores),
            ...
            map_value = map_metric.compute()
            aggregated_metrics["mAP"].append(map_value["map"].item())

    # Calculate precision-recall curve across the entire dataset
    precision, recall, _ = precision_recall_curve([1]*len(all_true_boxes), all_pred_scores)
    average_precision = average_precision_score([1]*len(all_true_boxes), all_pred_scores)
