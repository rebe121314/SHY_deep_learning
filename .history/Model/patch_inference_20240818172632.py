...
# Connect to Dropbox
ACCESS_TOKEN = 'sl.B7PGINFOJYqokwKVnajkz_NDG6pJEKuiD0oepzE21kA-DTwqPtcohgMINfgjQoJib56cwxI1jOGcV1R-zx6_FyTDElAvhL7xpLPKOl0uHsDaE6ogLyIf0Ehzpik8U-kdDIQ2q03TUpta'
...
class InferenceWithPatch:
    ...
    def label_relevant_cells(self, relevant_rows):
        ...
        for index, row in relevant_cells.iterrows():
            ...
            #if row['Tissue Category'] == 'Tumor' and row['Phenotype'] in pos_phen:
    ...
    def generate_bounding_boxes(self, patch, properties, pred_boxes, pred_scores, criteria_labels, i_offset, j_offset):
        ...
        for pred_box, score in zip(sorted_pred_boxes, sorted_pred_scores):
            ...
            # calculate the overlap between the box from properties and the box from the model
            ...
            for _, row in criteria_labels.iterrows():
                ...
                if expanded_minc <= x_position <= expanded_maxc and expanded_minr <= y_position <= expanded_maxr:
                    ...
                    ph = row['Phenotype']
                    ax.text(x_position - j_offset, y_position - i_offset, f'{ph:.2f}', color='black', fontsize=12, verticalalignment='top', bbox=dict(facecolor='white', alpha=0.5))
    ...
    def plot_cell_types(self, image, inform_excel):
        fig, ax = plt.subplots(figsize=(10, 10))
        ax.imshow(image)
        ...
        for index, row in inform_excel.iterrows():
            if row['Tissue Category'] == 'Tumor' and row['Phenotype'] in pos_phen:
                #ax.plot(row['Cell X Position'], row['Cell Y Position'], 'o', color='red', markersize=4)
                print(row['Phenotype'])
            elif row['Tissue Category'] == 'Blank':
                ax.plot(row['Cell X Position'], row['Cell Y Position'], 'o', color='blue', markersize=4)
            #elif row['Phenotype'] == 'Other cell':
            #    ax.plot(row['Cell X Position'], row['Cell Y Position'], 'o', color='green', markersize=4)
        '''
        self.model.eval()
        patch = image
        patch_tensor = F.to_tensor(patch).unsqueeze(0).to(self.device)
        ...
        for inform_file in inform_files:
            if inform_file.endswith('.xlsx'):
                ...
                if name_sample != example:
                    #print('Testing the tissue and cell types')
                    ...
                    #print('Continue with comparison')
                    continue
                ...
                for image_file in relevant_images:
                    ...
                    try:
                        ...
                        #self.plot_cell_types(gb_image, inform_excel)
