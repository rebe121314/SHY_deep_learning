...
def calculate_model_metrics(model, image, target, device, iou_threshold=0.5):
    metrics = {"tp": 0, "fp": 0, "fn": 0, "pred_scores": [], "true_labels": [], "iou": [], "mAP": 0.0}
    ...
    for pred_box in pred_boxes:
        ...
        for true_box in true_boxes:
            ...
            if iou > iou_threshold:
                tp += 1  # True Positive
                matched = True
                metrics["true_labels"].append(1)  # Positive label
                metrics["pred_scores"].append(pred_scores[pred_boxes.tolist().index(pred_box)])
                break
        if not matched:
            fp += 1  # False Positive
            metrics["true_labels"].append(0)  # Negative label
            metrics["pred_scores"].append(pred_scores[pred_boxes.tolist().index(pred_box)])
...
def model_evaluation(model, data_loader, device, iou_threshold=0.5):
    model.eval()
    aggregated_metrics = {"precision": [], "recall": [], "iou": [], "mAP": [], "pred_scores": [], "true_labels": []}
    ...
    for image_patches, target_patches in data_loader:
        for image, target in zip(image_patches, target_patches):
            ...
            aggregated_metrics["pred_scores"].extend(metrics["pred_scores"])
            aggregated_metrics["true_labels"].extend(metrics["true_labels"])

    # Calculate precision-recall curve
    precision, recall, _ = precision_recall_curve(aggregated_metrics["true_labels"], aggregated_metrics["pred_scores"])
    average_precision = average_precision_score(aggregated_metrics["true_labels"], aggregated_metrics["pred_scores"])
