...
class GranzymeBDataset(Dataset):
    ...
    def _load_images_and_labels(self):
        ...
        if self.val:
            ...
            label_files = label_files[:int(len_f*0.2)]
        else:
            # Use the rest of the samples for training
            #use 80% of the data for training
            len_f = len(label_files)

            label_files = label_files[int(len_f*0.8):]
...
def train(model, data_loader, val_loader, optimizer, scheduler, device, num_epochs):
    ...
    plot_iou_histogram(metrics["precision"])
...
def calculate_metrics(model, image, target, device):
    ...
    preds = [{
        ...
        "labels": torch.ones(len(prediction["boxes"])).int()  # Assuming single class (Granzyme B)
    }]

    targets = [{
        "boxes": target["boxes"].cpu(),
        "labels": torch.ones(len(target["boxes"])).int()  # Assuming single class (Granzyme B)
    }]

    # Update the metric with the current predictions and targets
    metric.update(preds, targets)

    # Compute the metrics
    result = metric.compute()

    # Extract the precision and recall metrics
    metrics["precision"].append(result['map'].item())
    metrics["recall"].append(result['mar_100'].item())
...
if __name__ == "__main__":
    ...
    loaded_label_folders = list_files_in_folder(labels_folder)
    ...
    load_path = "data/saved_models/extra_relax_basic_small.pth"

    #model.load_state_dict(torch.load(load_path))
    model.load_state_dict(torch.load(load_path, map_location=torch.device('cpu')))


    optimizer = torch.optim.SGD(model.parameters(), lr=0.005, momentum=0.9, weight_decay=0.0005) #lr = 0.0005 for 10 epochs fine tuning
    ...
    # Train the model
    train(model, data_loader,data_val_loader, optimizer, scheduler, device, num_epochs=50)
    #fused
    #simpie_flexible_only_training.pth
    #lablemanual_and_then_simpie
    save_path = "data/saved_models/50e_trans_with_relax.pth" 
