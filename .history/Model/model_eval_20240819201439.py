    ...
    for image_patches, target_patches in data_loader:
        for image, target in zip(image_patches, target_patches):
            ...
            preds = [{
                ...
                "labels": prediction["labels"].cpu()  # Ensure labels are used if more than one class
            }]

            targets = [{
                "boxes": target["boxes"].cpu(),
                "labels": target["labels"].cpu()  # Ensure labels are used if more than one class
            }]

            metric.update(preds, targets)

    # Compute final mAP and other metrics
    result = metric.compute()

    # Print out all the available metrics
    print(f"mAP: {result['map']:.4f}")
    print(f"mAP_50: {result['map_50']:.4f}")
    print(f"mAP_75: {result['map_75']:.4f}")
    print(f"Precision per class: {result['map_per_class']}")
    print(f"Recall per class: {result['mar_100_per_class']}")

    # Plot Precision-Recall curve
    plot_precision_recall_curve_new(result)

    # Plot IoU histogram
    plot_iou_histogram(result['map_50'])

    # Plot mAP histogram
    plot_map_histogram(result['map'])

    return result

def plot_precision_recall_curve_new(result):
    precision = result['map_per_class'].cpu().numpy()
    recall = result['mar_100_per_class'].cpu().numpy()

    if precision.size == 1 and recall.size == 1:
        print("Warning: Only one point for precision-recall curve.")
        precision = [precision]
        recall = [recall]

    plt.figure(figsize=(10, 5))
    plt.plot(recall, precision, marker='.', color='purple', label=f'mAP={result["map"]:.2f}')
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.title('Precision-Recall Curve')
    plt.legend()
...
def plot_iou_histogram(iou_values):
    if isinstance(iou_values, torch.Tensor):
        iou_values = iou_values.cpu().numpy()

    if len(iou_values.shape) == 0:  # Handle case where iou_values is a single value
        iou_values = [iou_values]

    plt.figure(figsize=(10, 5))
    plt.hist(iou_values, bins=50, color='purple', alpha=0.7)
    plt.xlabel('IoU')
    plt.ylabel('Frequency')
    plt.title('Distribution of IoU at 0.5 threshold')
    plt.show()
...
def plot_map_histogram(maps):
    ...
    if len(maps.shape) == 0:  # Handle case where maps is a single value
        maps = [maps]
