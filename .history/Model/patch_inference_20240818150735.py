...
class InferenceWithPatch:
    ...
    def generate_bounding_boxes(self, patch, properties, pred_boxes, pred_scores, criteria_labels, i_offset, j_offset):
        ...
        margin = 25  # Large margin to accommodate the possible bounding boxes
            ...
            for _, row in criteria_labels.iterrows():
                ...
                if expanded_minc <= x_position <= expanded_maxc and expanded_minr <= y_position <= expanded_maxr:
                    ...
                    if row['Phenotype'] == 'CD8':
                        ax.plot(x_position - j_offset, y_position - i_offset, 'o', color='red', markersize=4)
                    elif row['Phenotype'] == 'Tumor cell':
                        ax.plot(x_position - j_offset, y_position - i_offset, 'o', color='blue', markersize=4)
                    elif row['Phenotype'] == 'Other cell':
                        ax.plot(x_position - j_offset, y_position - i_offset, 'o', color='green', markersize=4)
