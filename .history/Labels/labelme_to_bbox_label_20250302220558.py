...
"""
Extracting Bounding Boxes from the LabelMe Annotations to Dropbox (central storage of the lab's image data ).

This script scans manually labeled JSON annotation files in Dropbox, extracts bounding boxes 
for specific cell types (e.g., Granzyme B-positive cells), and saves the results in a new 
Dropbox folder. 

- "w_GB": Weak Granzyme B signal.
- - "s_GB": Strong Granzyme B signal.
- "ps_GB": Pseudo positive Granzyme B signal.
- "fp_GB": False positive Granzyme B signal.
"""
def process_file(file_content, filename, dropbox_output_folder):
    ...
    for shape in data['shapes']:
        label = shape['label']
        #Predefined labels (`"w_GB", "s_GB", "ps_GB", "fp_GB"`)
        if label in ["w_GB", "s_GB", "ps_GB", "fp_GB"]: #Modify this line to change the data criteria. 
            points = shape['points']
          ...
          print("Couldn't't process the cell")
...
# Example usage
dropbox_annotation_folder = '/Lables/manual_json_labelme'
