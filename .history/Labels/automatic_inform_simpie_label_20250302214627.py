...
#For the script to function, it needs the access token for Dropbox (i.e., where the mages are stored)
load_dotenv()
...
class LabelGeneration:
    """
    This class automates the labeling of immune cells in immunohistochemistry (IHC) images. Uses cellpose to detect cells 
    ...
    def process_image(self, gb):
        ...
        #order the properties by the intesity of the staining
        properties = sorted(properties, key=lambda x: x.mean_intensity, reverse=True)
    ...
    def generate_bounding_boxes(self, patch, properties, criteria_labels, i_offset, j_offset):
        ...
        #order the criteria_labels by the granzyme b value, so the highest value is the first to be checke

        criteria_labels = criteria_labels.sort_values(by='Entire Cell Granzyme B (Opal 650) Mean (Normalized Counts, Total Weighting)', ascending=False)
        # order the properties by the intesity of the staining
                ...
                if name_sample in all_files_simpie:
                    ...
                    for image_file in relevant_images:
                        try:
                            ...
                            # add the simpie cells that are not in the criteria labels
                            add_criteria = simpie_cells[simpie_cells['Cell ID'] not in criteria_labels['Cell ID']]
    ...
    #automatic label every cell that has the basic criteria, menaing there's going to be false positives
