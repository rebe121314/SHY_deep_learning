...
# Connect to Dropbox
ACCESS_TOKEN = 'sl.B6O5a25I-e4rmwe8J_lEWERX9ZrhxEtu7uozxONNQ0wi-Re7hluCKtYBxx8ZtY4kMHUapGsLcphmBt1eCJdPCGHjQrx23JkHl1pFYmqcTAPuTs22FD9pZd6R2D7waeS-pSKOthl--w7W'
...
class LabelGeneration:
    ...
    def full_processing(self, image, patch_size, criteria_labels, simpie_cells, image_name):
        patches = self.create_patches_no_box(image, patch_size)
        patches_with_boxes = []

        for patch, i, j in patches:
            properties = self.process_image(patch)
            boxes = self.generate_bounding_boxes(patch, properties, criteria_labels, simpie_cells, i, j)
    ...
    def generate_bounding_boxes(self, patch, properties, criteria_labels, simpie_cells, i_offset, j_offset):
        boxes = []
        fig, ax = plt.subplots(figsize=(10, 10))
        ax.imshow(patch)
        ...
        relevant = criteria_labels[criteria_labels['Cell ID'].isin(simpie_cells['Cell ID'])]
        ...
        for inform_file in inform_files:
            ...
            if inform_file.endswith('.xlsx'):
                ...
                if name_sample in all_files_simpie:
                    ...
                    done_images = os.listdir('/Users/rebeca/Documents/Code/SHY_lab/desperate_test')
                    done_images = [f.split('_labels.json')[0] for f in done_images]
                    errors = os.listdir('/Users/rebeca/Documents/Code/SHY_lab/desperate_test/errors')
            ...
            if simp.endswith('.xlsx'):
                ...
                for image_file in relevant_images:
                    try:
                        ...
                        gb_image = self.read_image_from_dropbox(image_path)
                        patch_size = 256
                        self.full_processing(gb_image, patch_size, relevant_rows, relevant_rows, image_name)
                    except Exception as e:
                        self.errors.append({'image': image_file, 'error': str(e)})
                        print(f'Error processing {image_file}: {e}')
...
labels_folder = '/Labels/Simple_criteria_labels/'
